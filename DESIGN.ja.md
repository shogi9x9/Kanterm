...
- handoff結果（migration 0020）は成功時の完了noteをdurable result、失敗時のnoteを
  errorとして保存します。`get_handoff`でID取得でき、sender/status filterによりMCPから
  完了をpollできます。
- `watch-handoffs` は inbox item を claim し、command target または薄い bridge script
  へ配送します。named command / Cursor targetには、handoff dataをuntrusted inputとして
  delimiter分離したversion付きpacket control contractを配送します。`type: cursor`はpacketを
  Cursor Agent CLIのheadless prompt引数へ変換します。`type: interactive`の
  `adapter: kanpty`は同じpacketをstdinから`kanpty paste --enter`へ渡し、packet本文を
  process引数へ載せず、PTY / session lifecycleはKanpty側の責務に保ちます。tmux / zellij
  adapterは保留中です。
- **Agent work packet**: `kanterm-agent-work-packet/v1`は`orient`、`execute`、`verify`、
  `resume`で共有する決定的なenvelopeです。board / card本文はuntrusted dataとして
  delimiterで分離し、TUI clipboard previewと`run-agent-task`が同じcore rendererを
  呼びます。
- **試行監査ログ**（migration 0021）: 各自動試行の完全なpacket、profile、SHA-256、
  target、process結果、output、errorを保存します。retryのresume deltaは元packet digest、
  直近3試行、直近5 execution note、現在カードのacceptance dataに限定します。
- **完了gate**: targetのexit成功は`agent_succeeded`でありcard完了ではありません。
  verification commandがない場合や失敗時は同じhandoffをrequeueし、次のclaimでbounded
  resume deltaを使います。成功した場合だけhandoff完了、card archive、downstream
  workflow triggerを許可します。workflowはagent実行前にpreflightし、card完了後のworkflow
  dispatch保存失敗は現在handoffをterminal failureにして、agent作業をrequeueしません。
- **Target policy**: command target YAMLでdelivery、environment、network、workspace、
  approval、verification、writable pathを検証します。対応値はmachine-readableなadapter
  contractとして子processへ適用し、portable adapterが実装していないisolation指定は
  強制したように見せず失敗させます。
- **設定解決**: version付きmanifestのparseは`kanterm-core`が所有し、明示CLI path、
  `<repo>/.kanterm/config.yaml`、OS標準global manifestの順で解決します。相対file参照は
  宣言元manifest基準であり、board databaseの配置とは独立しています。
- 小さな workflow YAML は named step と `on_complete.send_handoff` を扱います。
  ...
  command target実行、verification、成功時だけのcard完了と次step triggerを一続きにします。
- Claude Code hook installer は Kanterm 所有の `SessionStart` / `SessionEnd` / `Stop`
  entry だけを管理し、他の hook を保持します。

## 0.3.0のdashboard改訂

- 有効なタブはKanban、LIST、TIMELINEです。`Tab` / `Shift+Tab`で巡回し、`1` / `2` /
  `3`で直接選択します。3画面すべてで`b`から同じboard switcherを開き、元のviewを
  保持します。
- FLOWは削除しました。TIMELINEは引き続き`dependency_stage_plan`のread projectionで、
  実行policyを所有しません。

## 実行ダッシュボード（0.2.0時点、上記改訂により置換）

- TUI はアクティブなボードの primary view から起動します。`Tab` / `Shift+Tab` で
  Kanban、LIST、TIMELINE、FLOWのタブを巡回し（`1` / `2` / `3` / `4`で直接選択）、
  running、human gate、ready、明示的 blocked、dependency waiting、
...
- `j` / `k` で実行リストを移動し、`Enter` で現在の実行タブに重ねてカード詳細を
  開きます。外部 MCP 更新は既存の `data_version` refresh でdashboardにも反映します。
- 実行viewはすべてアクティブなボード単位で表示します。TIMELINEはそのボードの
  `dependency_stage_plan`をガント風
  stage軸へ写し、calendar durationを仮定せず並列カードを同じ列に置きます。FLOWは
  そのボードの`classify_work`のlive countをstate-machine風mapへ置き、選択stateの
  カードを表示します。どちらも実行policyを所有せず、`kanterm-core`のread projection
  として扱います。
...
- 楽観的排他エラー時に、外部版の再読込かローカル編集の再開を選べる TUI recovery
  prompt を追加する。
- runtime 固有状態を `kanterm-core` に入れず、追加の tmux / zellij interactive
