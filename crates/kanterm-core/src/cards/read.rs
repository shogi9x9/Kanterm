...
use crate::rows::{
    row_to_card, CARD_COLUMNS, CARD_SELECT_BY_ID, CARD_SELECT_BY_KEY,
    CARD_SELECT_PREFIX_WHERE_BOARD,
};
...
impl Store {
    ...
    /// Every card on a board, including archived/completed history. Execution
    /// summaries use this to report progress without hiding finished work.
    pub fn cards_including_archived(&self, board_id: &str) -> Result<Vec<Card>> {
        let sql = format!(
            "SELECT {CARD_COLUMNS} FROM cards
             WHERE board_id = ?1 ORDER BY created_at, key_text"
        );
