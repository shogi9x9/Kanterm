...
- **エージェント・ネイティブ**: `kanban-mcp` がカード・列・ボード・memory log を
  エージェントに公開し、claim lease と検証フィールドで再開可能・監査可能な作業を
  支えます。
...
```sh
cargo build --release
./target/release/kanban-tui          # TUI を起動
```

プロジェクト内の [`.mcp.json`](.mcp.json) により、このディレクトリで実行すると
Claude Code が `kanban` MCP サーバを自動登録します。任意の MCP クライアントから
...
├─ kanban-tui    ratatui ボード。同期的な terminal UI。
└─ kanban-mcp    rmcp stdio MCP サーバ。非同期、エージェント向け。
```
...
`kanban-mcp` は stdio 経由でボードを AI エージェントに公開します。カードは key
...
memory log（`record_memory`・`recall_memories`）をカバーします。
