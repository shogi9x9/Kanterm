...
- **version付き agent work packet**: ボード把握、カード実行、検証、bounded resumeを
  1つの決定的なpacket形式にまとめ、TUIのコピーとcommand自動配送で共有します。
- **人間向け control plane**: アクティブなボードの実行listで running、human gate、ready、
  明示的 blocked、dependency waiting、context不足の作業を確認し、同じボードの
  dependency stage timelineで実行状況を集中して確認できます。
...
統合installerはplatformに合うKantermとKanptyのrelease archiveをdownloadし、公開済みの
SHA-256 checksumを両方検証してから、`kanterm`、`kanterm-mcp`、`kanpty`、`kanptyd`を
まとめてinstallします。
...
既定の配置先は`$HOME/.local/bin`です。変更する場合は`--install-dir PATH`を指定します。
Kantermは既定でlatest releaseを使い、Kanptyはprotocol v2互換releaseへpinします。
両方のversionを明示することもできます。
...
installerが行うのはbinary配置までです。`kanptyd`の起動やbackground service登録は
行いません。daemon lifecycleとservice managerはmachineごとに異なり、明示的に選択
できる状態を維持するためです。

Kantermだけを手動installする場合は、
[最新の GitHub Release](https://github.com/shogi9x9/Kanterm/releases/latest)から
platformに合うarchiveをdownloadします。
...
カードを開く、`n` で新規作成、`b` でボード切替、`q` で終了。Kanterm は実行
ダッシュボードから起動します。`Tab` / `Shift+Tab` でKanban、LIST、TIMELINEの
タブを巡回し、`1` / `2` / `3` で直接選択します。3画面すべてで`b`からボードを
切り替えられます。LISTとTIMELINEでは画面を離れず、`d`で選択カード、`D`で現在の
ボードをarchiveできます。フォーカス中の列・選択中の
カード・アクティブボードは起動間で記憶されます。実行タブでは`Enter`で現在のタブに
重ねてカード詳細を開き、`Esc`でKantermを終了します。カード詳細内の`Esc`はモーダル
だけを閉じます。
Kanban、LIST、TIMELINEで`C`を押すと現在ボードのorientation packetをpreviewし、
カード詳細の`C`では選択カードのexecution packetをpreviewします。表示後にのみ
`Enter` / `c` / `y`でclipboardへ送ります。
...
handoff送信元は`get_handoff`で完了結果を取得でき、一覧では送信済み・完了済みも
絞り込めます。
...
commandの成功が必要です。
target YAMLではCursor Agent CLIをheadless実行するfirst-classな`type: cursor` adapterと、
custom runtime向けの`type: command`を利用できます。
再利用するtarget / workflow pathはglobalまたは`<repo>/.kanterm/config.yaml`に保存でき、
`kanterm config path|show|init|edit|validate`でTUIを開かずに場所の確認と管理ができます。
