...
kanban-tui の改善にご協力いただき、ありがとうございます。
...
- `kanban-core` は SQLite のスキーマ、マイグレーション、ドメインルールを一元管理します。
- `kanban-tui` は対話系 UI を担当し、同期的に実装します。
- `kanban-mcp` は `kanban-core` の薄い MCP アダプタです。

データベースのロジックは `kanban-core` に集約し、TUI と MCP は
