...
| `list_handoffs` | recipient / sender / status でhandoffを列挙。既定はopenのみ |
| `get_handoff` | 1件のhandoffを完了結果または失敗error込みで全表示 |
...
`complete_handoff` で閉じます。完了時の`note`は永続result、失敗時の`note`はerrorに
保存されます。送信元は`list_handoffs(from_agent=..., status="completed")`で完了を
検知し、そのIDを`get_handoff(id=...)`へ渡して結果を取得できます。
lease があるため、受信側が動く前に終了しても別の
...
watcher は Kanterm DB を polling し、該当 handoff を claim して、claim 済みhandoffを
1件1行のJSONとしてstdoutへ出力します。stdoutと`--bridge-command`は配送専用で、受信
agentが結果付きで`complete_handoff`を呼ぶまでhandoffは`claimed`のままです。設定済み
command targetは同期実行され、stdoutをresultとして保存して`completed`になります。
hook / test用には`--once`で1回だけscanでき、`--interval-ms`でpolling intervalを変更
できます。
...
exit code 0は配送成功としてhandoffを`claimed`のまま維持し、非0ならbridge error付きで
`failed`になります。完了結果は受信agentが`complete_handoff`で登録します。
...
再利用するtarget / workflowのpathは、version付きmanifestで管理できます。
OS標準のglobal config directoryとrepository-local configを利用します。

- Global: `kanterm config path --global`が表示するpath（macOSでは
  `~/Library/Application Support/Kanterm/config.yaml`）
- Project: `<repo>/.kanterm/config.yaml`

`KANTERM_CONFIG_DIR`でglobal directoryを上書きできます。解決順は、明示したCLI flag、
project config、global configです。相対pathは、宣言したmanifestを基準に解決します。
...
以下のcommandはboard databaseやTUIを起動せずに実行できます。
...
`init`は既存manifestを上書きしません。`edit`は`VISUAL`、次に`EDITOR`を使い、
編集後の設定を検証します。YAMLはpathとruntime policy用であり、credentialやsecretは
保存しないでください。

`run-agent-task`、`watch-handoffs`、`run-workflow`は、対応するflagを省略した場合に
解決済みtarget / workflow pathを自動利用します。明示した`--targets` / `--workflow`が
常に最優先です。

配送先を繰り返し使う場合は、小さな YAML file に target を定義し、bridge 引数を
毎回書く代わりにnamed targetを選択できます。
...
`type: cursor`は`cursor-agent --print --output-format text --trust --workspace <repo>`へ
展開されます。Cursor CLIはgeneric targetのstdin契約を使わないため、完全なwork packetを
最後のprompt引数として渡します。`approval: external`では`--force`を付けず、変更適用を
外部actorへ残します。明示的なno-prompt policyである`approval: never`の場合だけ
`--force`を追加して直接変更を許可します。無人processを停止させる可能性がある
`approval: on-request`は拒否します。既存loginまたは`CURSOR_API_KEY`を利用できるよう、
Cursor targetは`environment: inherit`を必須とします。`model`は任意で`--model`へ対応します。
認証情報はoperatorが管理し、KantermはCursor credentialを保存しません。`cursor-agent`は
事前に`PATH`へ配置し、`cursor-agent login`または`CURSOR_API_KEY`で認証してください。
Cursor CLIはpromptをstdinではなく引数で受け取るため、実行中のpacketが同一hostの
process inspectionから見える場合があります。board / card本文へsecretを入れないでください。
...
`repo`基準で解決します。parent traversalを拒否し、既存path ancestorはsymlink解決後にも
repo内か検証します。portable command adapterが現在対応するのは
`network: inherit`と`workspace: repo-write`のみです。network denyやread-only workspaceは
実装済みのように扱わず、config parse時に失敗します。対応policyは
`KANTERM_DELIVERY_MODE`、`KANTERM_NETWORK_POLICY`、`KANTERM_WORKSPACE_POLICY`、
`KANTERM_APPROVAL_POLICY`、`KANTERM_WRITABLE_PATHS`として子processへ渡します。
これはadapter contractであり、OS sandbox自体はtarget command側の実装が必要です。
...
1 件 claim し、設定済みcommand targetを実行します。指定Kanterm cardを完了して次の
workflow stepをtriggerするのは、明示したverification commandが成功した場合だけです。
command出力はincoming handoffのresultにも保存され、送信元から取得できます。
...
runnerはstdinへ`kanterm-agent-work-packet/v1`を送ります。初回は`execute` profile、同じ
handoffのretryは、元packetのdigest、直近3試行、最大5件のexecution noteだけを含む
bounded `resume` profileです。migration 0021は各試行の完全なpacket、profile、SHA-256、
target、process結果、output、errorを保存します。

targetのexit codeとcard完了は別です。`--verify-command`がなければcardは
`verification_pending`に残し、handoffをrequeueします。verification失敗時は
`last_verification`を記録し、同じhandoffをrequeueして非0終了し、
`verification_failed`としてworkflowをtriggerしません。次の`run-agent-task`は同じhandoffを
再claimしてbounded `resume` packetを送ります。verificationが成功した場合だけcardと
handoffを完了し、次workflow stepへ進めます。workflowを指定した場合は、agent commandの
実行前にfile、step、target、render済みhandoffをpreflightします。その後のworkflow dispatch
保存だけが失敗した場合、完了済みagent作業はrequeueしません。cardはarchive済みのまま、
現在handoffを`failed`にして、修正後にworkflowを別途実行する必要があることをCLIで報告します。
